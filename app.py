...
# ----------------- ზუსტი Lovable/DIABLO CSS დიზაინი SVG იკონებით -----------------
st.markdown("""
<style>
    /* მთლიანი ფონი */
    .stApp {
    ...
    /* Bottom Navigation Bar (Streamlit Radio-ს ჩანაცვლება SVG იკონებით) */
    ...
    div[data-testid="stRadio"] label div[data-testid="stMarkdownContainer"] svg {
        fill: none;
        stroke: #64748b;
    }
    ...
    div[data-testid="stRadio"] label[aria-checked="true"] div[data-testid="stMarkdownContainer"] svg {
        stroke: #00dc82;
    }
...
# ----------------- 4. ASSETS -----------------
    ...
    st.markdown("<h4 style='margin-top:20px; font-size:15px;'>Accounts</h4>", unsafe_allow_html=True)
    
    st.markdown(f"""
    <div class="card-box">
        <small class="sub-text">Spot Account</small>
        <h3 style="margin:4px 0 0 0; font-size:18px;">0.00 USDT</h3>
    </div>
    <div class="card-box">
        <small class="sub-text">Futures Account</small>
        <h3 style="margin:4px 0 0 0; font-size:18px; color:#00dc82;">{st.session_state.balance:.2f} USDT</h3>
    </div>
    """, unsafe_allow_html=True)
